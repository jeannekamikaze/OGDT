...
struct Font
{
    ...
    font_render_mode mode;      // Current render mode.
};
...
Font* font_from_file (const char* alphabet_path, int w, int h, int stride, const char** err)
{
    Image img;
    image_from_file (&img, alphabet_path, err);
    if (*err) return 0;
    else return font_from_mem (img.pixels, w, h, stride);
}

Font* font_from_mem (const U8* pixels, int w, int h, int stride)
{
    Font* font = (Font*) malloc (sizeof(Font));
    assert (font);
    font->letter_stride  = stride;
    font->alphabet_width = w;

    GLuint tex;
    glGenTextures (1, &tex);
    glBindTexture (GL_TEXTURE_2D, tex);
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
    gluBuild2DMipmaps (GL_TEXTURE_2D, GL_ALPHA, w, h, GL_ALPHA, GL_UNSIGNED_BYTE, pixels);
...
void font_free (Font** font_ptr)
{
    Font* font = *font_ptr;
    if (font->tex) glDeleteTextures (1, &font->tex);
    free (font);
    *font_ptr = 0;
}

void font_start (Font* font, font_render_mode mode)
{
    if (!mode) mode == font_screen_space;
    ...
    font->mode = mode;
    if (mode == font_screen_space) enter2D();
}

void font_end (Font* font)
{
    if (font->mode == font_screen_space) leave2D();
...
void font_render (const Font* font, float px, float py, float pz,
                  const char* text, float font_size, font_draw_style style)
{
    if (font_size == 0.0f) font_size = 12.0f;
    if (!style) style = font_top_left;
    ...
    renderText(text, xpos, ypos, zpos, font->letter_stride, font->alphabet_width, font_size);
}

void font_render_to_aabb (const Font* font, float xmin, float ymin, float zmin,
                          float xmax, float ymax, float zmax, const char* text)
{
    ...
    float font_size   = fmin (font_width, font_height);
    ...
    renderText (text, xtl, ytl, ztl, font->letter_stride, font->alphabet_width, font_size);
}


void renderText (const char* text, float px, float py, float pz,
                 int letter_stride, int alphabet_width, float size)
{
    float starting_x = px;
